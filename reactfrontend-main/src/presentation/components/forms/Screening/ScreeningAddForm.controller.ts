 ...
 */
const getDefaultValues = (initialData?: ScreeningAddFormModel) => {
    const defaultValues = {
        startTime: new Date(),
        movieId: "",
        hallId: "",
        screenType: ScreenEnum.TwoD
    };
 ...
 */
export const useScreeningAddFormController = (onSubmit?: () => void): ScreeningAddFormController => {
    const { defaultValues, resolver } = useInitScreeningAddForm();
    const { mutateAsync: add, status } = useAddScreening();
    const queryClient = useQueryClient();
    const submit = useCallback((data: ScreeningAddFormModel) => // Create a submit callback to send the form data to the backend.
        add(data).then(() => {
            if (onSubmit) {
                onSubmit();
            }
    ...
    const {
        ...
        setValue,
        formState: { errors }
    } = useForm<ScreeningAddFormModel>({ // Use the useForm hook to get callbacks and variables to work with the form.
