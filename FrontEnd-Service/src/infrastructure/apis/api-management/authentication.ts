import {LoginRecord} from "../client/models/LoginRecord";
import {UserAddRecord} from "../client/models/UserAddRecord";
import {AuthorizationApi} from "../client/apis/AuthorizationApi";
...
export const useRegister = () => {
    return useMutation({ // Return the mutation object.
        mutationKey: [loginMutationKey], // Add the key to identify the mutation.
        mutationFn: (userAddRecord: UserAddRecord) => new AuthorizationApi().apiAuthorizationRegisterPost({userAddRecord}) // Add the mutation callback by using the generated client code and adapt it.
    })
}