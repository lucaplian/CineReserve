...
import {UserAddRecord} from "@infrastructure/apis/client/models/UserAddRecord";

import { useQueryClient } from "@tanstack/react-query";
import {useLogin, useRegister} from "@infrastructure/apis/api-management";
...
export const useRegisterFormController = (): RegisterFormController => {
    ...
    const { redirectToRegister } = useAppRouter();
    const { mutateAsync: register_, status } = useRegister();
    const queryClient = useQueryClient();
    const dispatch = useAppDispatch();
    const submit = useCallback((data: RegisterFormModel) => { // Create a submit callback to send the form data to the backend.
        const dataToRegister:  UserAddRecord=
            {name: data.name, email: data.email, password: data.password, role: "Client"};
        
        register_(dataToRegister).then((result:any) => {
            dispatch(setToken(result.response?.token ?? ''));
            toast(formatMessage({ id: "notifications.messages.registrationSuccess" }));
            redirectToHome();
        })}, [register_, redirectToHome, dispatch]);
    const {
        register,
        setValue,
        handleSubmit,
