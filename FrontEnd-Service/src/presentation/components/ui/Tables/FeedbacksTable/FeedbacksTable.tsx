...
import {UserRecord} from "@infrastructure/apis/client/models/UserRecord";
import { useGetMovie } from "@infrastructure/apis/api-management/movie";
    ...
    return <DataLoadingContainer isError={isError} isLoading={isLoading} tryReload={tryReload}> {/* Wrap the table into the loading container because data will be fetched from the backend and is not immediately available.*/}
        ...
        <DataTable data={pagedData?.data ?? []}
                   ...
                   extraHeader={[
                    {
                        ...
                        render: (entry: FeedbackRecord) => {
                        const {data} = useGetUser(entry.userId || ''); 
                    ...
                    },
                    {
                        ...
                        render: (entry: FeedbackRecord) => {
                        const {data} = useGetMovie(entry.movieId || ''); 
                    ...
                    }, order: 4},
                {
                       ...
                       render: entry => <>
                            {!isOpenDeleteModal && (isAdmin || isOwnUser?.id==entry.userId) &&   <IconButton color="primary" onClick={() => handleEditFeedback(entry)}>
                                <EditIcon color="primary" fontSize="small"/>
                            </IconButton>}
                           {!isOpenDeleteModal && (isAdmin || isOwnUser?.id==entry.userId)  && <IconButton color="error" onClick={() => handleDeleteUser(entry.id || '')}>
