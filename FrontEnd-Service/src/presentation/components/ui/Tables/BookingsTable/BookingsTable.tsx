...
import { useGetScreening } from "@infrastructure/apis/api-management/screening";
        ...
        <DataTable data={pagedData?.data ?? []}
                   ...
                   extraHeader={[
                    ...
                    },
                    {
                        ...
                        render: (entry: BookingRecord) => {
                            const {data} = useGetScreening(entry.screeningId || '');
                            return <>{data?.response?.movie.title}</>},
                        order: 1
                    },
                    {
                        key: "halls",
                        ...
                        render: (entry: BookingRecord) => {
                            const {data} = useGetScreening(entry.screeningId || '');
                            return <>{data?.response?.hall.name}</>},
                        order: 2
                    },
