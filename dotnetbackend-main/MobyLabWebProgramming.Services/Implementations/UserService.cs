    ...
    : IUserService
{
    ...
    public async Task<ServiceResponse<LoginResponseRecord>> RegisterUser(UserAddRecord user, CancellationToken cancellationToken = default)
    {
        ...
        await mailService.SendMail(user.Email, "Welcome!", MailTemplates.UserAddTemplate(user.Name), true, "CineReserve", cancellationToken); // You can send a notification on the user email. Change the email if you want.
        ...
        {
            User = userRecord,
            Token = loginService.GetToken(userRecord, DateTime.UtcNow, new(7, 0, 0, 0)) // Get a JWT for the user issued now and that expires in 7 days.
        });
