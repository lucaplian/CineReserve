...
public class FeedbackService(IRepository<WebAppDatabaseContext> repository): IFeedbackService
{
    ...
    public async Task<ServiceResponse> UpdateFeedback(FeedbackUpdateRecord feedback, UserRecord? requestingUser, CancellationToken cancellationToken = default)
    {
    ...
    public async Task<ServiceResponse> DeleteFeedback(Guid id, UserRecord? requestingUser = null, CancellationToken cancellationToken = default)
    {
        if (requestingUser != null && requestingUser.Role != UserRoleEnum.Admin)
        {
            return ServiceResponse.FromError(new(HttpStatusCode.Forbidden, "Only the admin can delete feedbacks!", ErrorCodes.CannotDelete));
        }
