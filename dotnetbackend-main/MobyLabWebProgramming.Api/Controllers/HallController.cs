...
public class HallController(ILogger<HallController> logger, IUserService userService, IHallService HallService) 
    : AuthorizedController(logger, userService)
{
    ...
    public async Task<ActionResult<RequestResponse<HallRecord>>> GetById([FromRoute] Guid id) // The FromRoute attribute will bind the id from the route to this parameter.
    {
        var currentUser = await GetCurrentUser();
        return currentUser.Result != null ? FromServiceResponse(await HallService.GetHall(id)) :
            ErrorMessageResult<HallRecord>(currentUser.Error);
    }
    ...
    {
        var currentUser = await GetCurrentUser();
        return currentUser.Result != null ? FromServiceResponse(await HallService.GetHalls(pagination)) :
            ErrorMessageResult<PagedResponse<HallRecord>>(currentUser.Error);
    ...
    public async Task<ActionResult<RequestResponse>> Add([FromBody] HallAddRecord Hall)
    {
        ...
        return currentUser.Result != null ?
            FromServiceResponse(await HallService.AddHall(Hall, currentUser.Result)) :
    ...
    public async Task<ActionResult<RequestResponse>> Update([FromBody] HallUpdateRecord Hall)  // The FromBody attribute indicates that the parameter is deserialized from the JSON body.
    {
        ...
        return currentUser.Result != null ?
            FromServiceResponse(await HallService.UpdateHall(Hall, currentUser.Result)) :
            ErrorMessageResult(currentUser.Error);
    }
    ...
    public async Task<ActionResult<RequestResponse>> Delete([FromRoute] Guid id) // The FromRoute attribute will bind the id from the route to this parameter.
    {
        ...
        return currentUser.Result != null ?
            FromServiceResponse(await HallService.DeleteHall(id, currentUser.Result)) :
            ErrorMessageResult(currentUser.Error);
    }
